...
test('dashboard registers /project-categories route that matches sidebar link', () => {
  ...
  assertContains(appSource, /import\s+ProjectCategories\s+from\s+['"]\.\/pages\/ProjectCategories['"]/, 'ProjectCategories page must be imported');
  assertContains(appSource, /path=["']project-categories["']\s+element=\{<ProjectCategories\s*\/>\}/, 'Dashboard router must register project-categories route');
